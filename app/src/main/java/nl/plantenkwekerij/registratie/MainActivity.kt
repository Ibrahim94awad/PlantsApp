...
@Composable private fun <T> Picker(label:String,values:List<T>,value:Long?,select:(T)->Unit,name:(T)->String){var open by remember{mutableStateOf(false)};var query by remember{mutableStateOf("")};val selected=values.firstOrNull{when(it){is Plant->it.id==value;is Department->it.id==value;is Line->it.id==value;is Size->it.id==value;else->false}};ExposedDropdownMenuBox(expanded=open,onExpandedChange={open=!open}){OutlinedTextField(selected?.let(name)?:"",{},Modifier.menuAnchor().fillMaxWidth(),label={Text(label)},readOnly=true,trailingIcon={ExposedDropdownMenuDefaults.TrailingIcon(open)});ExposedDropdownMenu(expanded=open,onDismissRequest={open=false}){OutlinedTextField(query,{query=it},label={Text("Zoeken...")},singleLine=true,modifier=Modifier.padding(8.dp).fillMaxWidth());val filtered=values.filter{name(it).contains(query,true)};if(filtered.isEmpty())DropdownMenuItem(text={Text("Geen items beschikbaar")},onClick={});filtered.forEach{x->DropdownMenuItem(text={Text(name(x))},onClick={select(x);open=false;query=""})}}}}
