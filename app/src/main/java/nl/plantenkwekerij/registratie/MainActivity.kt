...
class InventoryViewModel(private val db:PlantDatabase):ViewModel() {
 ...
 fun copy(id:Long){viewModelScope.launch{inventory.record(id)?.let{source->val stamp=System.currentTimeMillis();inventory.insert(source.copy(id=0,createdAt=stamp,updatedAt=stamp))}}}
...
@Composable private fun App(vm:InventoryViewModel){ var page by rememberSaveable{mutableStateOf(Page.LIST)}; var selected by rememberSaveable{mutableStateOf<Long?>(null)}
 when(page){Page.LIST->ListScreen(vm,{page=Page.FILTERS},{selected=null;page=Page.EDITOR},{selected=it;page=Page.EDITOR},{vm.copy(it)},{vm.delete(it)},{page=Page.MANAGEMENT}); Page.EDITOR->Editor(vm,selected,false,{page=Page.LIST}); Page.FILTERS->Filters(vm,{page=Page.LIST}); Page.MANAGEMENT->Management(vm,{page=Page.LIST})} }
...
@Composable private fun ListScreen(vm:InventoryViewModel,filters:()->Unit,new:()->Unit,edit:(Long)->Unit,copy:(Long)->Unit,delete:(Long)->Unit,manage:()->Unit){ val rows by vm.rows.collectAsStateWithLifecycle();val activeFilter by vm.filters.collectAsStateWithLifecycle();var confirm by remember{mutableStateOf<Long?>(null)};val filterCount=listOf(activeFilter.plantId,activeFilter.departmentId,activeFilter.lineId,activeFilter.sizeId).count{it!=null};val grouped=rows.groupBy{recordDate(it.createdAt)}
 Scaffold(topBar={Header("Registraties",actions={IconButton(onClick=filters){if(filterCount>0)BadgedBox(badge={Badge{Text(filterCount.toString())}}){Icon(Icons.Default.FilterList,"Filters")}else Icon(Icons.Default.FilterList,"Filters")};IconButton(onClick=manage){Icon(Icons.Default.Settings,"Beheer")}})},floatingActionButton={ExtendedFloatingActionButton(onClick=new,icon={Icon(Icons.Default.Add,null)},text={Text("Nieuwe registratie")})}){pad->Column(Modifier.padding(pad).fillMaxSize()){OutlinedTextField(vm.search.collectAsState().value,{vm.search.value=it},Modifier.padding(12.dp).fillMaxWidth(),label={Text("Zoeken...")},leadingIcon={Icon(Icons.Default.Search,null)},singleLine=true);if(filterCount>0)Surface(color=MaterialTheme.colorScheme.secondaryContainer,shape=MaterialTheme.shapes.medium,modifier=Modifier.padding(horizontal=12.dp).fillMaxWidth()){Row(Modifier.padding(start=12.dp),verticalAlignment=Alignment.CenterVertically){Icon(Icons.Default.FilterList,null);Text("  $filterCount filter${if(filterCount==1)"" else "s"} actief",Modifier.weight(1f));TextButton(onClick={vm.filters.value=InventoryFilter()}){Text("Wissen")}}};if(rows.isEmpty())Box(Modifier.fillMaxSize(),contentAlignment=Alignment.Center){Text("Nog geen registraties gevonden.")}else LazyColumn(contentPadding=PaddingValues(bottom=88.dp)){grouped.forEach{(date,dayRows)->item(key="day-$date"){Text("${dateLabel(date)} (${dayRows.size})",fontWeight=FontWeight.Bold,color=MaterialTheme.colorScheme.primary,modifier=Modifier.padding(start=16.dp,top=16.dp,bottom=4.dp))};items(dayRows,key={it.id}){r->Card(Modifier.padding(horizontal=12.dp,vertical=5.dp).fillMaxWidth()){Column(Modifier.padding(14.dp)){Row(Modifier.fillMaxWidth(),verticalAlignment=Alignment.CenterVertically){Text(r.plant,fontWeight=FontWeight.Bold,modifier=Modifier.weight(1f));Text("Nr. ${r.id}",style=MaterialTheme.typography.labelMedium,color=MaterialTheme.colorScheme.primary)};Text("${r.department}  •  ${r.line}  •  ${r.size}  •  ${r.quantity}");Text("Aangemaakt: ${format(r.createdAt)}",style=MaterialTheme.typography.labelSmall);Row{TextButton(onClick={edit(r.id)}){Text("Bewerken")};TextButton(onClick={copy(r.id)}){Text("Kopiëren")};TextButton(onClick={confirm=r.id}){Text("Verwijderen")}}}}}}}} }
...
private fun recordDate(value:Long)=Instant.ofEpochMilli(value).atZone(ZoneId.systemDefault()).toLocalDate()
private fun dateLabel(date:LocalDate)=when(date){LocalDate.now()->"Vandaag";LocalDate.now().minusDays(1)->"Gisteren";else->date.format(DateTimeFormatter.ofPattern("dd-MM-yyyy"))}

@Composable private fun Editor(vm:InventoryViewModel,id:Long?,isCopy:Boolean,back:()->Unit){ val plants by vm.plants.collectAsStateWithLifecycle(); val deps by vm.departments.collectAsStateWithLifecycle(); val lines by vm.lines.collectAsStateWithLifecycle(); val sizes by vm.sizes.collectAsStateWithLifecycle(); var old by remember { mutableStateOf<InventoryRecord?>(null) }
...
@Composable private fun <T> Picker(label:String,values:List<T>,value:Long?,select:(T)->Unit,name:(T)->String){var open by remember{mutableStateOf(false)};var query by remember{mutableStateOf("")};val selected=values.firstOrNull{when(it){is Plant->it.id==value;is Department->it.id==value;is Line->it.id==value;is Size->it.id==value;else->false}};Box(Modifier.fillMaxWidth()){OutlinedTextField(selected?.let(name)?:"",{},Modifier.fillMaxWidth(),label={Text(label)},readOnly=true,trailingIcon={Icon(Icons.Default.ArrowDropDown,null)});Box(Modifier.matchParentSize().clickable{open=true})};if(open){val requester=remember{FocusRequester()};val keyboard=LocalSoftwareKeyboardController.current;LaunchedEffect(Unit){delay(150);requester.requestFocus();keyboard?.show()};AlertDialog(onDismissRequest={open=false;query=""},title={Text("Kies $label")},text={Column{OutlinedTextField(query,{query=it},Modifier.fillMaxWidth().focusRequester(requester),label={Text("Zoeken...")},leadingIcon={Icon(Icons.Default.Search,null)},singleLine=true);Spacer(Modifier.height(8.dp));val filtered=values.filter{name(it).contains(query,true)};if(filtered.isEmpty())Text("Geen items beschikbaar",Modifier.padding(12.dp))else LazyColumn(Modifier.heightIn(max=360.dp)){items(filtered){x->ListItem(headlineContent={Text(name(x))},modifier=Modifier.clickable{select(x);open=false;query=""});HorizontalDivider()}}}},confirmButton={TextButton(onClick={open=false;query=""}){Text("Sluiten")}})}}

@Composable private fun Filters(vm:InventoryViewModel,back:()->Unit){val p by vm.plants.collectAsStateWithLifecycle();val d by vm.departments.collectAsStateWithLifecycle();val l by vm.lines.collectAsStateWithLifecycle();val s by vm.sizes.collectAsStateWithLifecycle();var f by remember { mutableStateOf(vm.filters.value) };Scaffold(topBar={Header("Filters",back)}){pad->Column(Modifier.padding(pad).padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){Picker("Plant",p,f.plantId,{f=f.copy(plantId=it.id)},{it.name});Picker("Afdeling",d,f.departmentId,{f=f.copy(departmentId=it.id)},{it.name});Picker("Lijn",l,f.lineId,{f=f.copy(lineId=it.id)},{it.name});Picker("Maat",s,f.sizeId,{f=f.copy(sizeId=it.id)},{it.name});Button(onClick={vm.filters.value=f;back()},Modifier.fillMaxWidth()){Text("Filters toepassen")};TextButton(onClick={vm.filters.value=InventoryFilter();back()},Modifier.fillMaxWidth()){Text("Filters wissen")}}}}
...
@Composable private fun Management(vm:InventoryViewModel,back:()->Unit){var section by remember{mutableStateOf("Planten")};var name by remember{mutableStateOf("")};var qty by remember{mutableStateOf("")};var message by remember{mutableStateOf<String?>(null)};var editing by remember{mutableStateOf<Triple<Long,String,Int?>?>(null)};val values:List<Triple<Long,String,Int?>> = when(section){"Planten"->vm.plants.collectAsStateWithLifecycle().value.map{Triple(it.id,it.name,null)};"Afdelingen"->vm.departments.collectAsStateWithLifecycle().value.map{Triple(it.id,it.name,null)};"Lijnen"->vm.lines.collectAsStateWithLifecycle().value.map{Triple(it.id,it.name,null)};else->vm.sizes.collectAsStateWithLifecycle().value.map{Triple(it.id,it.name,it.defaultQuantity)}}
 Scaffold(topBar={Header("Beheer",back)}){pad->Column(Modifier.padding(pad).padding(16.dp)){val tabs=listOf("Planten","Afdelingen","Lijnen","Maten");ScrollableTabRow(selectedTabIndex=tabs.indexOf(section),edgePadding=0.dp){tabs.forEach{x->Tab(selected=section==x,onClick={section=x;name="";qty=""},text={Text(x,maxLines=1)})}};Spacer(Modifier.height(12.dp));OutlinedTextField(name,{name=it},Modifier.fillMaxWidth(),label={Text("Naam")},singleLine=true);if(section=="Maten")OutlinedTextField(qty,{qty=it.filter(Char::isDigit)},Modifier.fillMaxWidth(),label={Text("Standaardaantal")},singleLine=true);Button(onClick={if(name.isNotBlank()&&(section!="Maten"||qty.toIntOrNull()!=null)){vm.add(section,name.trim(),qty.toIntOrNull()?:0);name="";qty=""}},Modifier.fillMaxWidth()){Text("Toevoegen")};message?.let{Text(it,color=MaterialTheme.colorScheme.error)};LazyColumn{items(values,key={it.first}){item->ListItem(headlineContent={Text(if(item.third==null)item.second else "${item.second} (${item.third})")},trailingContent={Row{IconButton(onClick={editing=item}){Icon(Icons.Default.Edit,"Bewerken")};IconButton(onClick={vm.deleteMaster(section,item.first){message="Dit item wordt gebruikt in bestaande registraties en kan niet worden verwijderd."}}){Icon(Icons.Default.Delete,"Verwijderen")}}});HorizontalDivider()}}}}
