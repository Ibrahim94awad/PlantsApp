...
@Dao interface InventoryDao {
    @Query("""SELECT r.id, p.name plant, d.name department, l.name line, s.name size, r.quantity, r.createdAt, r.updatedAt FROM inventory_records r JOIN plants p ON p.id=r.plantId JOIN departments d ON d.id=r.departmentId JOIN lines l ON l.id=r.lineId JOIN sizes s ON s.id=r.sizeId WHERE (:search='' OR lower(p.name) LIKE '%' || lower(:search) || '%' OR lower(d.name) LIKE '%' || lower(:search) || '%' OR lower(l.name) LIKE '%' || lower(:search) || '%' OR lower(s.name) LIKE '%' || lower(:search) || '%') AND (:plantId IS NULL OR r.plantId=:plantId) AND (:departmentId IS NULL OR r.departmentId=:departmentId) AND (:lineId IS NULL OR r.lineId=:lineId) AND (:sizeId IS NULL OR r.sizeId=:sizeId) ORDER BY r.createdAt DESC""") fun rows(search: String, plantId: Long?, departmentId: Long?, lineId: Long?, sizeId: Long?): Flow<List<InventoryRow>>
