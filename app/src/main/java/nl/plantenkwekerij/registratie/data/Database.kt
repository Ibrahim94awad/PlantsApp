...
@Entity(tableName = "subdepartments", indices = [Index(value = ["name"], unique = true)]) data class SubDepartment(@PrimaryKey(autoGenerate = true) val id: Long = 0, val name: String, val createdAt: Long = now(), val updatedAt: Long = now())
...
@Entity(tableName = "inventory_records", indices = [Index("plantId"), Index("departmentId"), Index("subDepartmentId"), Index("lineId"), Index("sizeId")], foreignKeys = [ForeignKey(entity = Plant::class,parentColumns=["id"],childColumns=["plantId"],onDelete=ForeignKey.RESTRICT), ForeignKey(entity=Department::class,parentColumns=["id"],childColumns=["departmentId"],onDelete=ForeignKey.RESTRICT), ForeignKey(entity=SubDepartment::class,parentColumns=["id"],childColumns=["subDepartmentId"],onDelete=ForeignKey.RESTRICT), ForeignKey(entity=Line::class,parentColumns=["id"],childColumns=["lineId"],onDelete=ForeignKey.RESTRICT), ForeignKey(entity=Size::class,parentColumns=["id"],childColumns=["sizeId"],onDelete=ForeignKey.RESTRICT)]) data class InventoryRecord(@PrimaryKey(autoGenerate = true) val id: Long = 0, val plantId: Long, val departmentId: Long, val subDepartmentId: Long, val lineId: Long, val sizeId: Long, val quantity: Int, val createdAt: Long = now(), val updatedAt: Long = now())

data class InventoryRow(val id: Long, val plant: String, val department: String, val subDepartment: String, val line: String, val size: String, val quantity: Int, val createdAt: Long, val updatedAt: Long)
data class InventoryFilter(val plantId: Long? = null, val departmentId: Long? = null, val subDepartmentId: Long? = null, val lineId: Long? = null, val sizeId: Long? = null)

@Dao interface InventoryDao {
   @Query("""SELECT r.id, p.name plant, d.name department, sd.name subDepartment, l.name line, s.name size, r.quantity, r.createdAt, r.updatedAt FROM inventory_records r JOIN plants p ON p.id=r.plantId JOIN departments d ON d.id=r.departmentId JOIN subdepartments sd ON sd.id=r.subDepartmentId JOIN lines l ON l.id=r.lineId JOIN sizes s ON s.id=r.sizeId WHERE (:search='' OR lower(p.name) LIKE '%' || lower(:search) || '%' OR lower(d.name) LIKE '%' || lower(:search) || '%' OR lower(sd.name) LIKE '%' || lower(:search) || '%' OR lower(l.name) LIKE '%' || lower(:search) || '%' OR lower(s.name) LIKE '%' || lower(:search) || '%') AND (:plantId IS NULL OR r.plantId=:plantId) AND (:departmentId IS NULL OR r.departmentId=:departmentId) AND (:subDepartmentId IS NULL OR r.subDepartmentId=:subDepartmentId) AND (:lineId IS NULL OR r.lineId=:lineId) AND (:sizeId IS NULL OR r.sizeId=:sizeId) ORDER BY r.createdAt DESC""") fun rows(search: String, plantId: Long?, departmentId: Long?, subDepartmentId: Long?, lineId: Long?, sizeId: Long?): Flow<List<InventoryRow>>
   ...
   @Query("SELECT COUNT(*) FROM inventory_records WHERE subDepartmentId=:id") suspend fun subDepartmentUsage(id:Long):Int
...
@Dao interface MasterDao {
   ...
   @Query("SELECT * FROM subdepartments WHERE name LIKE '%' || :q || '%' COLLATE NOCASE ORDER BY name") fun subDepartments(q:String=""):Flow<List<SubDepartment>>; @Insert(onConflict=OnConflictStrategy.IGNORE) suspend fun addSubDepartment(x:SubDepartment):Long; @Update suspend fun updateSubDepartment(x:SubDepartment); @Delete suspend fun deleteSubDepartment(x:SubDepartment)
...
@Database(entities=[Plant::class,Department::class,SubDepartment::class,Line::class,Size::class,InventoryRecord::class],version=3,exportSchema=true)
abstract class PlantDatabase:RoomDatabase(){ abstract fun inventory():InventoryDao; abstract fun master():MasterDao
 companion object {
  ...
  private fun seed(db:SupportSQLiteDatabase,context:Context){
   ...
   if(count(db,"subdepartments")==0)db.execSQL("INSERT OR IGNORE INTO subdepartments(name,createdAt,updatedAt) VALUES(?,?,?)",arrayOf("Algemeen",now(),now()))
  ...
  private val MIGRATION_2_TO_3 = object : Migration(2,3) {
     override fun migrate(db:SupportSQLiteDatabase) {
       db.execSQL("CREATE TABLE IF NOT EXISTS subdepartments (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, createdAt INTEGER NOT NULL, updatedAt INTEGER NOT NULL)")
       val nowValue = now()
       db.execSQL("INSERT OR IGNORE INTO subdepartments(name, createdAt, updatedAt) VALUES(?, ?, ?)", arrayOf("Algemeen", nowValue, nowValue))
       db.execSQL("ALTER TABLE inventory_records ADD COLUMN subDepartmentId INTEGER NOT NULL DEFAULT 0")
       db.execSQL("UPDATE inventory_records SET subDepartmentId = (SELECT id FROM subdepartments WHERE name = 'Algemeen' LIMIT 1)")
       db.execSQL("CREATE INDEX IF NOT EXISTS index_inventory_records_subDepartmentId ON inventory_records(subDepartmentId)")
     }
  }
  fun open(context:Context)=Room.databaseBuilder(context,PlantDatabase::class.java,"plantregistratie.db").addMigrations(object:Migration(1,2){override fun migrate(db:SupportSQLiteDatabase){db.execSQL("CREATE INDEX IF NOT EXISTS index_inventory_records_plantId ON inventory_records(plantId)")}}, MIGRATION_2_TO_3).addCallback(object:Callback(){override fun onCreate(db:SupportSQLiteDatabase){super.onCreate(db);seed(db,context)};override fun onOpen(db:SupportSQLiteDatabase){super.onOpen(db);seed(db,context)}}).build()
 }
}
