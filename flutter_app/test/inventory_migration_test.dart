...
void main() {
  ...
  test('migreert dubbele registraties veilig naar voorraad en geschiedenis',
      () async {
    sqfliteFfiInit();
    ...
    final legacy = await openDatabase(
      ...
      onCreate: (db, version) async {
        await db.execute(
            'CREATE TABLE plants(id INTEGER PRIMARY KEY, name TEXT NOT NULL UNIQUE, createdAt INTEGER NOT NULL, updatedAt INTEGER NOT NULL)');
        await db.execute(
            'CREATE TABLE departments(id INTEGER PRIMARY KEY, name TEXT NOT NULL UNIQUE, createdAt INTEGER NOT NULL, updatedAt INTEGER NOT NULL)');
        await db.execute(
            'CREATE TABLE lines(id INTEGER PRIMARY KEY, name TEXT NOT NULL, departmentId INTEGER, createdAt INTEGER NOT NULL, updatedAt INTEGER NOT NULL)');
        await db.execute(
            'CREATE TABLE sizes(id INTEGER PRIMARY KEY, name TEXT NOT NULL UNIQUE, defaultQuantity INTEGER NOT NULL, createdAt INTEGER NOT NULL, updatedAt INTEGER NOT NULL)');
    ...
    await legacy.insert('departments', {
      'id': 1,
      'name': 'A6',
      'createdAt': firstStamp,
      'updatedAt': firstStamp
    });
    await legacy.insert('lines', {
      'id': 1,
      'name': 'Lijn 1',
      'departmentId': null,
      'createdAt': firstStamp,
    ...
    expect(
        migratedHistory
            .map((entry) => entry.changeAmount)
            .reduce((a, b) => a + b),
        356);
    expect(migratedHistory.where((entry) => entry.action == 'created'),
        hasLength(1));

    await appDatabase.saveRecord(
        plantId: 1,
        departmentId: 1,
        subDepartmentId: 1,
        lineId: 1,
